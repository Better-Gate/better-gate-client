...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "better-gate-client-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "better-gate-client-sync",
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "better-gate-client-sync";
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "better-gate-client-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "better-gate-client-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <Input
              ...
              placeholder="better-gate-client-sync"
              className="text-xs flex-1"
              disabled={isLoading}
            />
          </div>
